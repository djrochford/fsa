...
class DFA:
    """A deterministic finite-state automaton class. Takes three parameters: a transition function, a start state 
    ...
    The fsa expects the symbols of the alphabet to be one character strings. States can be anything hashable. (Note that,
    ...
    def _good_range(self):
        ...
        if transition_range - self.states != set():
    ...
    def __or__(self, dfa):
        """Takes an dfa instance as a parameter. Returns an dfa instance that recognizes the language A union B, 
        where A is the language recognized byself and B the language recognized by the input dfa."""
...
class FSA:
    ...
    You can also define epsilon-moves; use the empty string in place of an alphabet symbol in the transition function to do so.
